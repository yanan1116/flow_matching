...
class LiberoWindowedDataset(Dataset):
    ...
    def __init__(self, base_ds, horizon=16, obs_horizon=1, normalize_images_01=True, task_map=None, include_task_text=True):
        self.base = base_ds
        self.H = int(horizon)
        self.O = int(obs_horizon)
        assert self.H > 0 and self.O > 0
        self.normalize_images_01 = normalize_images_01
        self.include_task_text = bool(include_task_text)
        
        # task_map: {task_index(int) -> natural language instruction(str)}
        self.task_map = task_map
        if self.include_task_text:
...
ds = LiberoWindowedDataset(base_ds, 
                           ...
                           task_map=task_map,
                           include_task_text=(not args.disable_text_input))

collate_fn = collate_with_task_text if not args.disable_text_input else None
...
for epoch in tqdm(range( args.num_epochs ), desc="Training Epochs"):
    ...
    if (not args.disable_text_input) and args.frozen_text_model:
        nets['text_encoder'].eval()
    ...
    for ii, batch in enumerate(pbar):
        ...
        if args.debug and (not args.disable_text_input):
    ...
    if args.save_cp and epoch in eval_epoch_milestones: # save checkpint at some intervals
        ...
        if (not args.disable_text_input) and (not args.frozen_text_model):
